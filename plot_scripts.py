...
def make_video_as_avi(test, test_p, norm, to_img, target_fps=30, f_s=None, filename='movies/UD-test.avi', cmap=None):
    RMS_time = np.mean((test.y - test_p.y)**2, axis=(2, 3))**0.5
    NRMS_time = np.mean(RMS_time / norm.ystd[:, 0, 0], axis=1) if norm is not None else RMS_time
    DT = f_s is None
    f_s = 1 if f_s is None else f_s
    ...
    ani = FuncAnimation(fig, update, frames=tqdm(range(len(test.y))), init_func=init, blit=True, interval=1000 / target_fps)

    # Use PillowWriter to save as .gif
    gif_writer = PillowWriter(fps=target_fps)
    ani.save(filename, writer=gif_writer)
